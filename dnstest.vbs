...
if (objFSO.fileexists("C:\temp\DNSTEST")) then              ''LOGFILE EXISTS
  objFSO.deletefile "C:\temp\DNSTEST", true
  set objLOG = objFSO.createtextfile("C:\temp\DNSTEST")
  objLOG.close
  set objLOG = objFSO.opentextfile("C:\temp\DNSTEST", 8)
...
if (errRET = 0) then                                          ''ARGUMENTS PASSED, CONTINUE SCRIPT
  objOUT.write vbnewline & now & " - STARTING DNSTEST - " & strIP & vbnewline
  objLOG.write vbnewline & now & " - STARTING DNSTEST - " & strIP & vbnewline
	''AUTOMATIC UPDATE, DNSTEST.VBS, REF #2 , REF #69 , REF #68
  objOUT.write vbnewline & now & vbtab & vbtab & " - CHECKING FOR UPDATE : DNSTEST : " & strVER
  objLOG.write vbnewline & now & vbtab & vbtab & " - CHECKING FOR UPDATE : DNSTEST : " & strVER
  ...
  if ((intRET = 4) or (intRET = 10) or (intRET = 11) or (intRET = 1) or (intRET = 2147221505) or (intRET = 2147221517)) then
    ...
    while (strLOOP = vbnullstring)
      objOUT.write vbnewline & now & vbtab & vbtab & " - DNSTEST : " & strIP
      objLOG.write vbnewline & now & vbtab & vbtab & " - DNSTEST : " & strIP
...
sub CLEANUP()                                 			        ''SCRIPT CLEANUP
  on error resume next
  if (errRET = 0) then         											        ''DNSTEST COMPLETED SUCCESSFULLY
    objOUT.write vbnewline & vbnewline & now & vbtab & " - DNSTEST SUCCESSFUL : " & now
    objLOG.write vbnewline & vbnewline & now & vbtab & " - DNSTEST SUCCESSFUL : " & now
    err.clear
  elseif (errRET <> 0) then    											        ''DNSTEST FAILED
    objOUT.write vbnewline & vbnewline & now & vbtab & " - DNSTEST FAILURE : " & errRET & " : " & now
    objLOG.write vbnewline & vbnewline & now & vbtab & " - DNSTEST FAILURE : " & errRET & " : " & now
    ''RAISE CUSTOMIZED ERROR CODE, ERROR CODE WILL BE DEFINE RESTOP NUMBER INDICATING WHICH SECTION FAILED
    call err.raise(vbObjectError + errRET, "DNSTEST", "FAILURE")
  end if
  objOUT.write vbnewline & vbnewline & now & " - DNSTEST COMPLETE" & vbnewline
  objLOG.write vbnewline & vbnewline & now & " - DNSTEST COMPLETE" & vbnewline
  objLOG.close
  ''EMPTY OBJECTS
  set objLOG = nothing
  set objFSO = nothing
