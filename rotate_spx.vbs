...
''DEFINE VARIABLES
dim objWSH, objFSO, objSPX, colFIL, objFIL
...
if (wscript.arguments.count > 0) then        ''ARGUMENTS WERE PASSED
  ...
  if (wscript.arguments.count > 1) then
    ...
    objOUT.write vbnewline & now & vbtab & "SCRIPT REQUIRES PATH TO LOCAL SHADOWPROTECT BACKUP DESTINATION"
    call CLEANUP
end if
''RETRIEVE SHADOWPROTECT BACKUPSET FOLDER
set objSPX = objFSO.getfolder(strFOL)
''CHECK THROUGH ALL FILES IN SHADOWPROTECT BACKUPSET FOLDER
''THIS NEEDS TO BE THE COMPLETE PATH THAT SHADOWPROTECT BACKUPS WRITE TO I.E "E:\Backups"
set colFIL = objSPX.files
...
sub CLEANUP()                                ''SCRIPT CLEANUP
  if (errRET = 0) then                       ''NO ERROR RETURNED
    ...
    wscript.stdout.write vbnewline & vbtab & "ROTATE SHADOWPROTECT BACKUPSET : COMPLETE : CLEARED : " & lngSIZ & "GB : " & Now()
  elseif (errRET <> 0) then                  ''ERROR RETURNED
    objOUT.write vbnewline & vbtab & "ROTATE SHADOWPROTECT BACKUPSET : ERROR : " & Now()
    ''RAISE CUSTOMIZED ERROR CODE, ERROR CODE WILL BE DEFINE RESTOP NUMBER INDICATING WHICH SECTION FAILED
    call err.raise(vbObjectError + errRET,"ROTATE SHADOWPROTECT BACKUPSET", "FAIL")
  end if
  ''EMPTY OBJECTS
  set objFIL = nothing
  set colFIL = nothing
